...
(function () {
    ...
    async function fetchData() {
        ...
        if (blynkToken) {
            ...
            // Simulate acceleration: mostly calm, occasionally a shock spike
            const accelSpike = Math.random() < 0.08;
            if (accelSpike) {
                ax = +(0.8 + Math.random() * 1.0).toFixed(2);
                ay = +(0.6 + Math.random() * 0.8).toFixed(2);
                az = +(1.2 + Math.random() * 0.8).toFixed(2);
            } else {
    ...
    function checkAlerts(temp, accelMag, tilt, ldr, boxText) {
        ...
        if (accelMag > thresholds.accelMax) {
            alerts.push({ sensor: 'acceleration', msg: `High Acceleration Detected: ${accelMag.toFixed(2)}g — Possible shock or drop event (threshold: ${thresholds.accelMax}g)` });
